...
// Regen entire meal (all foods except locked ones)
function regenMeal(mealId) {
  ...
  if (LOCKS.meals[mealId]) return; // skip if meal locked

  const oldMeal = plan.meals[mealIndex];

  // Build prePlaced array from locked foods in this meal
  const prePlacedItems = oldMeal.items.filter(it => LOCKS.foods[it._uid]);

  // Use rebuildMeal to regenerate only unlocked foods
  const newPlan = rebuildMeal(plan, mealIndex, window._lastOpts, {
    mealsByIndex: { [mealIndex]: prePlacedItems }
  });
...
function regenFood(mealId, itemId) {
  ...
  // Pick a new portion for the same slot
  const newItem = pickPortion(meal.slot || oldItem.slot, window._lastOpts);
  if (!newItem) return;

  // Preserve the original _uid and set base macros
  ...
  // Recalculate macros based on qty
  ...
  // Recalculate meal and day totals
  recalcMeal(meal);
  recalcTotals(plan);
  syncLocks(plan);

  // Update the global plan and UI
  window._lastPlan = plan;
  renderResult(plan);
}
