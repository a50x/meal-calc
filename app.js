...
function buildCandidate(mealCount){
  ...
  for(let m=0;m<mealCount;m++){
    let mealItems=[], itemCount = rand(1,3), attempts = 0;
...
function generateDay(){
  ...
  if(mealCountChoice==='optimal'){
    ...
    const cand = findBestForMealCount(m, params);
    if(!cand){ 
      document.getElementById('result').innerHTML = `<div class="card"><strong>No plan found for ${m} meals.</strong></div>`; 
      return;
    }
...
fetch('foods.json')
  .then(res => res.json())
  .then(data => { FOODS = data; generateDay(); });
